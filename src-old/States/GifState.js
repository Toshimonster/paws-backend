...
export class GifState extends BaseState {
    ...
    gifDefinitions = [];
    start;
    constructor(name, options, transitions) {
    ...
    executeStateFrame(driver, t, dt, ddt) {
        let gifTime = 0;
...
class GifFrame {
    ...
    ;
    constructor(gif, frame, transform) {
...
class Gif {
    ...
    gifReader;
    transform;
    constructor(gifReader, transform = 'normal') {
    ...
    static fromDataBuffer(buffer, transform = 'normal') {
        return new Gif(new GifReader(buffer), transform);
    }
    static fromFile(file, transform = 'normal') {
        return Gif.fromDataBuffer(readFileSync(file), transform);
    }
}
//# sourceMappingURL=GifState.js.map